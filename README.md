...
* **Projekte speichern & exportieren**
  * **Autosave** im Browser – nach jeder Änderung (entprellt), beim nächsten Besuch ist alles wieder da
  * **⬇ Projekt** schreibt eine `.vdp.json` mit Noten, Drum-Pattern, Synth-Patch, Tempo und Tonart
  * **⬇ MIDI** exportiert eine Standard-MIDI-Datei (Format 1): Spur 1 Melodie, Spur 2 Drums auf
    GM-Kanal 10 – direkt in Ableton, FL Studio, Logic oder Reaper ladbar
  * **⬆ Laden** liest Projektdateien wieder ein; defekte oder manipulierte Dateien werden
    validiert und bereinigt statt die App zu zerstören
...
Getestet werden Musiktheorie (Akkordauflösung in allen 12 Tonarten), Level- und
Bewertungslogik (jedes Level ist nachweislich lösbar), Euclidean-Rhythmen gegen die
kanonischen Muster, das Freischalt-System der Regler sowie die UI inklusive
Piano Roll, Drum-Sequencer und Fortschrittsspeicherung.

Dazu für Speichern/Laden: Validierung beliebiger Fremddaten (defektes JSON, manipulierte
Werte, Prototype Pollution, Pfad-Traversal in Dateinamen), Byte-genaue Prüfung der
erzeugten MIDI-Dateien, Autosave-Entprellung, Ressourcen-Aufräumen beim Unmount sowie
Performance- und Stresstests (10.000 Noten exportieren in ~33 ms).
