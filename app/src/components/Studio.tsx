...
export function Studio() {
  ...
  const [pattern, setPattern] = useState<DrumPattern>(() => emptyPattern(64))
  const [recMode, setRecMode] = useState<'off' | 'overdub' | 'replace'>('off')
  const [armNextLoop, setArmNextLoop] = useState(false)
  const [quantize, setQuantize] = useState(true)
  const [patch, setPatch] = useState<PatchParams>({ ...DEFAULT_PATCH, osc: 'sawtooth', cutoff: 3000, attack: 0.02, release: 0.4, reverb: 0.25, delay: 0.15 })
  ...
  const currentStep = () => {
    const t = Tone.getTransport()
    const raw = t.ticks / (Tone.getTransport().PPQ / 4)
    return (quantize ? Math.round(raw) : raw) % steps
  }
  ...
  const noteOn = useCallback(
    (midi: number, vel = 0.8) => {
      ...
      if (playing && recRef.current !== 'off') heldRef.current.set(midi, { start: currentStep(), vel })
    },
    [playing, quantize, steps],
  )
  ...
  const noteOff = useCallback((midi: number) => {
    ...
    if (held) {
      ...
      const len = Math.max(1, quantize ? Math.max(1, Math.round(end - held.start)) : end - held.start)
      setNotes((ns) => [...ns, { id: crypto.randomUUID(), midi, start: Math.floor(held.start) % steps, len: Math.min(len, steps), vel: held.vel }])
    }
  }, [quantize, steps])
  ...
  const { devices, enabled, error, connect } = useMidi((e) => {
    if (e.type === 'noteon') {
      ...
      if (pad && e.note < 68 && e.note >= 36 && padMode) {
  ...
  const euclid = (pulses: number, len: number) => {
    const out = Array(len).fill(0)
    for (let i = 0; i < pulses; i++) out[Math.floor((i * len) / pulses)] = 1
    return out
  }
  ...
  const shift = (dir: number) =>
    setPattern((p) => {
      const n = { ...p }
      for (const d of DRUM_ORDER) {
        const head = p[d].slice(0, steps)
        const k = ((dir % steps) + steps) % steps
        n[d] = head.slice(steps - k).concat(head.slice(0, steps - k)).concat(Array(64 - steps).fill(0))
      }
      return n
    })
  ...
  return (
    <div className="studio">
      <div className="topbar">
        ...
        <label className="inline">BPM <input type="number" min={40} max={200} value={bpm} onChange={(e) => setBpm(Number(e.target.value))} /></label>
        <label className="inline">Takte
          <select value={bars} onChange={(e) => setBars(Number(e.target.value))}>{[1, 2, 4].map((b) => <option key={b}>{b}</option>)}</select>
        </label>
      ...
      <div className="grid-2">
        ...
        <section className="card">
          <h3>MIDI-Controller (z.B. Nektar Impact)</h3>
      ...
      <section className="card">
        <h3>Piano Roll <small>{notes.length} Noten · Schritt {pos + 1}/{steps}</small></h3>
      ...
      <section className="card">
        <h3>Drum Machine <small>8 Pads · Euclidean & Mutation</small></h3>
      ...
      <section className="card">
        <h3>Sound Design <small>derselbe Synth wie im Ear-Trainer</small></h3>
        <div className="panels">
          {(['Oscillator', 'Filter', 'Amp Envelope', 'Filter Envelope', 'EQ', 'FX'] as const).map((grp) => (
