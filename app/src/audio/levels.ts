...
export type Level = {
  ...
  presetKey?: string
  /** consolidation level: teaches no new knob, combines what came before */
  review?: boolean
}
...
export const LEVELS: Level[] = [
  ...
  { id: 6, chapter: 'Grundlagen', title: 'Volle ADSR-Kontrolle', lesson: 'Jetzt alle vier Hüllkurven-Stufen gleichzeitig treffen.', review: true, params: ['osc', 'cutoff', 'attack', 'decay', 'sustain', 'release'] },
  ...
  { id: 9, chapter: 'Filter', title: 'Filter-Sweep timing', lesson: 'F-Attack und F-Decay bestimmen, wie schnell der Sweep passiert.', tip: 'Langer F-Attack = der Sound "öffnet" sich langsam.', review: true, params: ['osc', 'cutoff', 'resonance', 'filterEnvAmount', 'fAttack', 'fDecay', 'attack', 'sustain', 'release'] },
  ...
  { id: 17, chapter: 'Modulation', title: 'Modulations-Routing', lesson: 'Jetzt alle drei LFO-Ziele gleichzeitig. Welches Ziel hörst du wirklich?', tip: 'Pitch = Tonhöhe wackelt, Filter = Helligkeit wackelt, Amp = Lautstärke wackelt.', review: true, params: ['osc', 'lfoRate', 'lfoToPitch', 'lfoToFilter', 'lfoToAmp', 'cutoff', 'resonance'] },
...
/** Snap a raw value onto the knob's step grid and clamp it into range.
 *  Rounding alone could push a value below min (e.g. attack 0.001 -> 0),
 *  which produced -Infinity for log params and unreachable targets. */
export function quantize(spec: ParamSpec, raw: number): number {
  const min = spec.min!
  const max = spec.max!
  const step = spec.step ?? 0.01
  const snapped = spec.log && raw >= 100 ? Math.round(raw) : Math.round(raw / step) * step
...
export function randomTarget(level: Level): PatchParams {
  ...
  const set = (id: ParamId, raw: number) => {
    ;(t as Record<string, unknown>)[id] = quantize(PARAM_SPECS[id], raw)
  }
  ...
  for (const id of level.params) {
    ...
    if (s.kind === 'choice') {
      ...
      set(id, Math.exp(rnd(Math.log(s.min!), Math.log(s.max!))))
    } else {
      set(id, rnd(s.min!, s.max!))
    }
  }

  // make modulation levels clearly audible instead of near-zero
  const focused = level.params.length <= 7
  if (level.params.includes('lfoToPitch') && focused && t.lfoToPitch < 20) set('lfoToPitch', 25 + Math.random() * 60)
  if (level.params.includes('lfoToFilter') && focused && t.lfoToFilter < 0.25) set('lfoToFilter', 0.3 + Math.random() * 0.6)
  if (level.params.includes('lfoToAmp') && focused && t.lfoToAmp < 0.25) set('lfoToAmp', 0.3 + Math.random() * 0.6)
  if (level.params.includes('noiseLevel') && t.noiseLevel < 0.2) set('noiseLevel', 0.3 + Math.random() * 0.6)
  if (level.params.includes('delayTime') && t.delay < 0.25) set('delay', 0.3 + Math.random() * 0.4)
  return t
}
